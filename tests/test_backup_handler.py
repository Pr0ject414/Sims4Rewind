...
def test_initial_backup_creation(temp_dirs, mocker):
    ...
    handler = BackupHandler(str(saves_dir), str(backup_dir), 5, mock_status_cb, mock_created_cb, mock_pruned_cb, mocker.MagicMock(), mocker.MagicMock())
...
def test_backup_on_modification(temp_dirs, mocker):
    ...
    handler = BackupHandler(str(saves_dir), str(backup_dir), 5, mocker.MagicMock(), mocker.MagicMock(), mocker.MagicMock(), mocker.MagicMock(), mocker.MagicMock())
    handler._is_running = True # FIX: Simulate the running state for the test
    handler._initialize_and_create_initial_backups()
    assert len(os.listdir(backup_dir)) == 1
...
def test_backup_skipped_for_unchanged_content(temp_dirs, mocker):
    ...
    # Arrange: Run the initial backup
    handler = BackupHandler(str(saves_dir), str(backup_dir), 5, mock_status_cb, mocker.MagicMock(), mocker.MagicMock(), mocker.MagicMock(), mocker.MagicMock())
    handler._is_running = True # FIX: Simulate the running state for the test
    handler._initialize_and_create_initial_backups()
    assert len(os.listdir(backup_dir)) == 1
...
def test_backup_pruning(temp_dirs, mocker):
    ...
    handler = BackupHandler(str(saves_dir), str(backup_dir), backup_count, mocker.MagicMock(), mocker.MagicMock(), mock_pruned_cb, mocker.MagicMock(), mocker.MagicMock())
